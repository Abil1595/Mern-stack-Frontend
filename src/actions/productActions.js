...
export const getProducts = (keyword, price, category, rating, currentPage, brands, offerprice) => async (dispatch) => {
    try {
        ...
        // Add brands to the URL if they are provided
        if (brands && brands.length > 0) {
            link += `&Brand=${brands.join(',')}`; // Join selected brands with a comma
        }

        // Add offerprice to the URL if provided
        if (offerprice) {
            link += `&offerprice=${offerprice}`; // Include offer price in the request
        }
